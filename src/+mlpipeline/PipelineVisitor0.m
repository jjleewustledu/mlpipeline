classdef PipelineVisitor0
    ...
    methods (Static)
        ...
        function xfm   = thisOnThatXfmFilename(varargin)
            xfm = [mlpipeline.PipelineVisitor0.thisOnThatImageFilename(varargin{:}) mlfsl.FlirtVisitor.XFM_SUFFIX];
        end
        function xfm   = thisOnThatDatFilename(varargin)
            xfm = mlpipeline.PipelineVisitor0.thisOnThatImageFilename(varargin{:});
        ...
        function pth   = guessSessionPath
            pth = pwd;     
            import mlpipeline.*;
            for w = 1:length(PipelineVisitor0.WORKFOLDERS)
                pos = strfind(pth, PipelineVisitor0.WORKFOLDERS{w});
                if (~isempty(pos))
                    pth = pth(1:pos-2);
                end
            end
	...
	methods 
 		function this = PipelineVisitor0(varargin) 
 			%% PIPELINEVISITOR 
 			%  Usage:  this = PipelineVisitor0() 
            ...
            addParameter(p, 'sessionPath', PipelineVisitor0.guessSessionPath, @(v) lexist(v, 'dir'));
            addParameter(p, 'workPath',    PipelineVisitor0.guessWorkpath,    @(v) lexist(v, 'dir')); 
            parse(p, varargin{:});
